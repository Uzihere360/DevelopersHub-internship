...
module.exports = function login () {
  ...
  function afterLogin (user: { data: User, bid: number }, res: Response, next: NextFunction) {
    ...
    BasketModel.findOrCreate({ where: { UserId: user.data.id } })
      .then(([basket]: [BasketModel, boolean]) => {
        const token = jwt.sign({ id: user.data.id, email: user.data.email }, 'your-secret-key', { expiresIn: '1h' })
        user.bid = basket.id
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const authenticatedUser = await models.User.findOne({ where: { email: email, deletedAt: null } })
  ...
  function verifyPreLoginChallenges (req: Request) {
    ...
    challengeUtils.solveIf(challenges.exposedCredentialsChallenge, () => {
      return req.body.email === 'testing@' + config.get<string>('application.domain') && req.body.password === 'IamUsedForTesting'
    })
  }
